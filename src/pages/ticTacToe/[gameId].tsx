...
const TicTacToe: NextPage = () => {
        ...
        return board.map((row, i: number) => {
            return row.map((cell, j: number) => {
                return (
                    ...
                    >
                        <h2 className="">
                            {isNaN(cell) ? '?' : map[getBoardValue(i, j)]}
                        </h2>
                    </button>
                );
            });
    ...
    const gameStateQuery = api.ticTacToe.getBoard.useQuery(
        { gameId: gameId },
        {
            onSuccess: (data) => setBoard(transformBoard(data?.board)),
        }
    );
    ...
    useEffect(() => {
        ...
        //diagonal wins
        if (
            (getBoardValue(0, 0) + getBoardValue(1, 1) + getBoardValue(2, 2)) %
                3 ===
            0
        ) {
            return setWinner(turn);
        ...
        if (
            (getBoardValue(0, 2) + getBoardValue(1, 1) + getBoardValue(2, 0)) %
                3 ===
            0
        ) {
            return setWinner(turn);
            ...
            <div>
                ...
                {isNaN(winner) && gameStateQuery.isSuccess && (
                    <h2 className="text-center text-4xl">
                        <h2>
                            {JSON.stringify(
                                transformBoard(gameStateQuery.data.board)
                            )}
                        </h2>
                    </h2>
                )}
                <button onClick={async () => await gameStateQuery.refetch()}>
                    Refresh Board
                </button>
            </div>
        </div>
