...
export const ticTacToeRouter = createTRPCRouter({
  ...
  .mutation( async ({ ctx, input: {playerCookie, xLocation, yLocation, gameId}}) => {
    ...
    if (game.player1 === playerCookie) { playerNumber = 1 }
    else if (game.player2 === playerCookie) { playerNumber = 2 }
    else { throw Error('Player does not exist in this game') }
   
    //check turn
    if (playerNumber !== game.currentTurn) {throw Error('Other player\'s turn')}

    // check if square is empty
    if (!game.board[index]) { throw Error('Square is not empty'); }
    
    const playerSymbol = playerNumber === 1 ? 'X' : 'O';
    const newBoard = game.board.slice(0, index) + playerSymbol + game.board.slice(index + 1);
    return ctx.prisma.game.update({where: {gameId: game.gameId}, data: {board: newBoard, currentTurn: 3 - game.currentTurn}});
  })
});
